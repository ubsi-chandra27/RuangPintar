...
async function main() {
  const [sekolah, guru, siswa, rombel, mapel, sesi, presensiHadir, presensiIzin, presensiAlpha] = await Promise.all([
    prisma.sekolah.findMany({ select: { id: true, nama: true, tipe_lisensi: true } }),
    prisma.pengguna.findMany({ where: { peran_dasar: "TEACHER" }, select: { id: true, nama_lengkap: true, email: true } }),
    prisma.siswa.count(),
    prisma.rombel.findMany({
      ...
      include: {
        penempatan_rombel: {
          select: { id: true, keikutsertaan: { select: { siswa: { select: { nama_lengkap: true } } } } }
        },
        penugasan_mengajar: {
  ...
  rombel.forEach(r => {
    const mapelList = r.penugasan_mengajar.map(j => `${j.mata_pelajaran?.nama} (${j.guru?.nama_lengkap})`).filter(Boolean);
    console.log(`- ${r.nama} (Tingkat ${r.tingkat_kelas}): ${r.penempatan_rombel.length} siswa, Mapel: ${[...new Set(mapelList)].join(", ")}`);
  });
  console.log("Presensi:", { Hadir: presensiHadir, Izin: presensiIzin, Alpha: presensiAlpha, TotalSesi: sesi });
}
