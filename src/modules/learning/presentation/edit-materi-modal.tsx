...
/**
 * Ruang Pintar — M11 Edit Materi Pembelajaran Modal
...
import { updateMateriAction } from "@/app/actions/learning-actions";
import { LingkupMateriDTO, MateriPembelajaranDTO } from "../domain/learning-types";

interface EditMateriModalProps {
  penugasanId: string;
  materi: MateriPembelajaranDTO | null;
...
export function EditMateriModal({
  penugasanId,
  materi,
  lingkupMateriList,
  isOpen,
  ...
  onSuccess,
  onError,
}: EditMateriModalProps) {
  const [tipeKonten, setTipeKonten] = useState<"DOKUMEN" | "TEKS" | "TAUTAN" | "VIDEO">(
    (materi?.tipe_konten as any) || "DOKUMEN"
  );
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [prevMateriId, setPrevMateriId] = useState(materi?.id);
  const [isPending, startTransition] = useTransition();

  if (materi && materi.id !== prevMateriId) {
    setPrevMateriId(materi.id);
    setTipeKonten((materi.tipe_konten as any) || "DOKUMEN");
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.set("id", materi.id);
    formData.set("penugasan_mengajar_id", penugasanId);
    formData.set("tipe_konten", tipeKonten);
    formData.set("existing_berkas_id", materi.berkas_id || "");

    startTransition(async () => {
      const res = await updateMateriAction(null, formData);
      if (res.success) {
      ...
      >
        <div className="p-4 sm:p-5 border-b border-slate-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">Edit Materi Pembelajaran</h3>
              <p className="text-xs text-slate-500">Perbarui modul, bacaan, atau tautan materi</p>
            </div>
          </div>
          <button
            type="button"
        ...
        >
          <div>
            ...
            <select
              ...
              defaultValue={materi.lingkup_materi_id || ""}
          ...
          {tipeKonten === "DOKUMEN" && (
            <div className="space-y-2">
              ...
              {materi.berkas_id && !selectedFile && (
                <div className="flex items-center justify-between p-2.5 rounded-xl bg-blue-50/70 border border-blue-100 text-xs">
                  <div className="flex items-center gap-2 text-blue-800 font-semibold truncate">
                    <FileCheck className="h-4 w-4 text-[#2563EB] shrink-0" />
                    <span className="truncate">Dokumen saat ini telah terunggah</span>
                  </div>
                  <a
                    href={`/api/berkas/${materi.berkas_id}`}
                    target="_blank"
                    rel="noreferrer"
                    className="text-[11px] font-bold text-[#2563EB] hover:underline shrink-0 ml-2"
                  >
                    Lihat Berkas
                  </a>
                </div>
              )}
              <div className="relative border-2 border-dashed border-slate-200 hover:border-blue-400 rounded-2xl p-4 transition-colors text-center bg-slate-50/60 hover:bg-blue-50/30">
                <input
                  type="file"
                  name="file"
                ...
                <div className="flex flex-col items-center justify-center pointer-events-none">
                  <div className="p-2.5 rounded-xl bg-blue-50 text-[#2563EB] mb-1.5 shadow-2xs">
                  ...
                  {selectedFile ? (
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5">
                        {(selectedFile.size / 1024 / 1024).toFixed(2)} MB • Berkas baru akan
                    ...
                    <div>
                      <p className="text-xs font-bold text-slate-700">
                        {materi.berkas_id
                          ? "Unggah berkas baru untuk mengganti (opsional)"
          ...
          {tipeKonten === "TAUTAN" && (
            <div>
              ...
              <input
                type="url"
                name="tautan_url"
                defaultValue={materi.tautan_url || ""}
                required
          ...
          {tipeKonten === "TEKS" && (
            <div>
              ...
              <textarea
                name="konten_teks"
                defaultValue={materi.konten_teks || ""}
                rows={5}
                required
